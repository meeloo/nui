...
public:
  ...
  // Cells settings:
  void SetCellSpacing(uint32 cell, nuiSize hgap); ///< Sets the horizontal spacing to hgap around widgets in the given cell.
  void SetAllCellsSpacing(nuiSize gap);  //< Sets the horizontal spacing to gap around widgets in all cells.
  ...
  void SetCellPixels(uint32 column, nuiSize size); ///< Force the size of a column.
  void ResetCellPixels(uint32 cell);  ///< make the minimum size in pixels back to its variable capactiy

  void SetSize(nuiSize size); ///< Set the requested height of the whole box
  
  // Corresponding Getters:
  nuiSize GetCellSpacing(uint32 cell) const; ///< Gets the horizontal spacing around widgets in the given cell.
