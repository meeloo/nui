...
//    NGL_LOG("nglInternalDataObject", NGL_LOG_DEBUG, "0x%p - CTOR(%s / %d / 0x%p)\n", this, rMimeType.GetChars(), mNativeType, data);//#FIXME
  }
  ...
  virtual void AllocFormatData(FORMATETC * pFormat, STGMEDIUM * pMedium)
  {
    ...
    NGL_LOG("nglInternalDataObject", NGL_LOG_DEBUG, "0x%p - AllocFormatData(0x%p / 0x%p)\n", this, pMedium->hGlobal, mData);//#FIXME
  }
  ...
  virtual void SetFormatData(FORMATETC * pFormat, STGMEDIUM * pMedium)
  {
    ...
    *dataPtr = mData;
    NGL_LOG("nglInternalDataObject", NGL_LOG_DEBUG, "0x%p - SetFormatData(0x%p / 0x%p)\n", this, pMedium->hGlobal, dataPtr);//#FIXME
  ...
  virtual void GetFormatData(FORMATETC* pFormat, STGMEDIUM* pMedium)
  {
    C* dataPtr = (C*)GlobalLock(pMedium->hGlobal); 
    mData = *dataPtr;
    NGL_LOG("nglInternalDataObject", NGL_LOG_DEBUG, "0x%p - GetFormatData(0x%p / 0x%p)\n", this, pMedium->hGlobal, dataPtr);//#FIXME
    GlobalUnlock(pMedium->hGlobal);
  }
#endif
  ...
  virtual void GetFlavorData(DragRef dragRef, DragItemRef& itemRef, FlavorType flavorType)
  {
    ...
    if (err != noErr)
    {
      NGL_LOG("nglInternalDataObject", NGL_LOG_ERROR, "Error GetFlavorDataSize: code %d\n", err);
      NGL_ASSERT(0);
    }

    //NGL_LOG("nglInternalDataObject", NGL_LOG_DEBUG, "ReceiveData: dataSize: %d\n", dataSize);
