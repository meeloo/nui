...
public:
	...
	*/
	KIWI_ERROR_RETURN_TYPE addConstraint( const Constraint& constraint )
	{
		if( m_cns.find( constraint ) != m_cns.end() )
			KIWI_THROW( DuplicateConstraint, constraint )
        ...
        KIWI_THROW( UnsatisfiableConstraint, constraint )
			else
				subject = tag.marker;
		}
	...
	*/
	KIWI_ERROR_RETURN_TYPE removeConstraint( const Constraint& constraint )
	{
		...
		if( cn_it == m_cns.end() )
      KIWI_THROW( UnknownConstraint, constraint )
	...
	*/
	KIWI_ERROR_RETURN_TYPE addEditVariable( const Variable& variable, double strength )
	{
		if( m_edits.find( variable ) != m_edits.end() )
			KIWI_THROW( DuplicateEditVariable, variable )
		strength = strength::clip( strength );
		if( strength == strength::required )
			KIWI_THROW_( BadRequiredStrength )
	...
	*/
	KIWI_ERROR_RETURN_TYPE removeEditVariable( const Variable& variable )
	{
		EditMap::iterator it = m_edits.find( variable );
		if( it == m_edits.end() )
			KIWI_THROW( UnknownEditVariable, variable )
	...
	*/
	KIWI_ERROR_RETURN_TYPE suggestValue( const Variable& variable, double value )
	{
		EditMap::iterator it = m_edits.find( variable );
		if( it == m_edits.end() )
			KIWI_THROW( UnknownEditVariable, variable )
...
private:
	...
	*/
	KIWI_ERROR_RETURN_TYPE optimize( const Row& objective )
	{
		while( true )
		{
	...
	*/
	KIWI_ERROR_RETURN_TYPE dualOptimize()
	{
		while( !m_infeasible_rows.empty() )
		{
			...
			if( it != m_rows.end() && it->second->constant() < 0.0 )
			{
				...
				if( entering.type() == Symbol::Invalid )
					KIWI_THROW( InternalSolverError, "Dual optimize failed." )
				// pivot the entering symbol into the basis
				Row* row = it->second;
				m_rows.erase( it );
				row->solveFor( leaving, entering );
