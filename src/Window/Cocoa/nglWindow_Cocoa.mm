...
NGL_OUT("[T:%s] Window resized to %d x %d (content scale = %f)\n", nglThread::GetCurThreadName().GetChars(), (int)clientRect.size.width, (int)clientRect.size.height, (float)layer.contentsScale);
NGL_OUT("[T:%s] Changed drawable size to %d x %d\n", nglThread::GetCurThreadName().GetChars(), (int)layer.drawableSize.width, (int)layer.drawableSize.height);
...
{
  NGL_OUT("[T:%s] nglWindow::CreateMetalPass()\n", nglThread::GetCurThreadName().GetChars());
  ...
  if (!mMetalDrawable)
  {
    ...
    NGL_OUT("[T:%s] Next drawable size is %d x %d (layer: %d x %d [%f] / window:%d x %d)\n", nglThread::GetCurThreadName().GetChars(), (int)texture.width, (int)texture.height, (int)layer.frame.size.width, (int)layer.frame.size.height, (float)layer.contentsScale, GetWidth(), GetHeight());
