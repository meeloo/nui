...
bool nglNativeVolume::GetChildren(const nglPath& rPath, std::vector<nglPath>& rChildren)
{
  nglPath p(mInfos.mPath);
  ...
  std::vector<nglPath>::iterator it = rChildren.begin();
  std::vector<nglPath>::iterator end = rChildren.end();
